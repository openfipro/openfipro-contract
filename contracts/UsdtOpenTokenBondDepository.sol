...
contract UsdtOpenTokenBondDepository is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    struct Bond {
        uint256 payout; // 【OpenToken remaining to be paid】
    ...
    /* ======== STATE VARIABLES ======== */
    address public OpenToken; // token given as payment for bond
    address public principle; // token used to create bond
    address public treasury; // mints OpenToken when receives principle
    ...
    address public stakingAddr; // to auto-stake payout
    address public stakingHelper; // to stake and claim if no stakingAddr warmup
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 OpenToken ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) OpenToken
         */
        ...
        if (fee != 0) {
            ...
            IERC20(OpenToken).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(OpenToken).transfer(_recipient, _amount); // send payout
        } else {
            // 【if user wants to stake】
            if (useHelper) {
                //【use if stakingAddr warmup is 0】
                IERC20(OpenToken).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(OpenToken).approve(stakingAddr, _amount);
                IStaking(stakingAddr).stakeOpenToken(
                    _amount,
                    _recipient,
                    defaultRebaseRateInfoId(),
                    0
                );
                // IStaking(stakingAddr).stake(_amount, _recipient);
            }
        }
        return _amount;
    }
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(OpenToken).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(OpenToken).totalSupply();
        // uint256 currentDebt = currentDebt().mul(1e9);
        // uint256 currentDebtToLittleValue = currentDebt.div(10);
    ...
    function defaultRebaseRateInfoId() public view returns (uint256) {
        return IStakingConfig(stakingConfig).defaultRebaseRateInfoId();
    }
